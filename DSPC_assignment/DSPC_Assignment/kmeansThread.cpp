...
namespace KmeansCluster {
    ...
    void KmeansThread::addToClosest(MovieData::Movie& amovie) {
    ...
    float KmeansThread::predict(MovieData::Movie aMovie) {
        //get the distance from the centroid to the county
        float distance1 = distanceFromCluster(aMovie, cluster1);
        float distance2 = distanceFromCluster(aMovie, cluster2);
        float distance3 = distanceFromCluster(aMovie, cluster3);
    ...
    void KmeansThread::initialize(vector<Movie> movies) {
        //initialize random
        srand(time(NULL));
    ...
    vector<float> KmeansThread::mean(std::vector<Movie>& cluster) {
    ...
    Movie KmeansThread::getCentroid(std::vector<Movie>& cluster, vector<float> mean) {
        if (cluster.empty()) {
    ...
    void KmeansThread::cluster() {
        ...
        for (int i = 0; i < totalThreads; ++i) {
            threads.push_back(std::thread(processCluster, i));
        }
    ...
    float KmeansThread::avgDistance(vector<Movie>& cluster, int index) {
    ...
    float KmeansThread::distanceFromCluster(Movie& c, vector<Movie>& cluster) {
        //cumilate distance
        float distance = 0;
        for (Movie& a : cluster) {
            distance += c - a;
    ...
    float KmeansThread::silh(vector<Movie>& a, vector<Movie>& b, int index) {
